    ...
    while True:
        ...
        for x in range(0, 4):
            ...
            for block in snake_body[1:]:
                if positions[x][0] == block[0] and positions[x][1] == block[1]:
                    val[x] = 0
        SP = [np.sign(food_pos[0]-snake_pos[0])+1, np.sign(snake_pos[1]-food_pos[1])+1, val[0], val[1], val[2], val[3], A]

        #update value function
        Q[S[0], S[1], S[2], S[3], S[4], S[5], S[6], A] += alpha*(R + gamma*np.argmax(Q[SP[0], SP[1], SP[2], SP[3], SP[4], SP[5], SP[6], :]) - Q[S[0], S[1], S[2], S[3], S[4], S[5], S[6], A])
        model[S[0], S[1], S[2], S[3], S[4], S[5], S[6], A] = [SP[0], SP[1], SP[2], SP[3], SP[4], SP[5], SP[6], R]
        pairs.append([S[0], S[1], S[2], S[3], S[4], S[5], S[6], A])
        #update n random previous states
        n = 0
        while n < 10:
            rand = random.randint(0, len(pairs)-1)
            S = [pairs[rand][0], pairs[rand][1], pairs[rand][2], pairs[rand][3], pairs[rand][4], pairs[rand][5], pairs[rand][6]]
            A = pairs[rand][7]
            temp = model[S[0], S[1], S[2], S[3], S[4], S[5], S[6], A]
            SP = [temp[0], temp[1], temp[2], temp[3], temp[4], temp[5], temp[6]]
